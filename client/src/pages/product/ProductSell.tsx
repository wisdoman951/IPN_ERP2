    ...
    const groupedSales = useMemo(() => {
        ...
        orderGrouped.forEach((items) => {
            ...
            if (isBundleGroup) {
                ...
                const orderedNoteLines: string[] = [];
                const appendLine = (line: string) => {
                    const trimmed = line.trim();
                    if (!trimmed) {
                        return;
                    }
                    if (!orderedNoteLines.includes(trimmed)) {
                        orderedNoteLines.push(trimmed);
                    }
                };

                productQuantityMap.forEach((quantity, name) => {
                    appendLine(quantity > 0 ? `${name} x${quantity}` : name);
                });
                manualNoteSet.forEach((line) => appendLine(line));

                base.combined_note = orderedNoteLines.length > 0 ? orderedNoteLines.join("\n") : undefined;
                base.bundle_metadata = undefined;
            }
